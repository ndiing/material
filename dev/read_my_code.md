                ...
                <thead>
                    <tr>
                        ${this.weekdays.map(weekday=>html`
                            <th
                                class="${classMap({
                                    'md-datetime--start-of-week': weekday.startOfWeek
                                })}"
                            >${weekday.label}</th>
                        `)}
                    </tr>
                    ...
                    ${this.calendar.map(row=>html`
                        <tr>
                            ${row.children.map(cell=>html`
                                <td
                                    class="${classMap({
                                        'md-datetime--start-of-week': cell.startOfWeek,
                                        'md-datetime--outside-month': cell.outsideMonth,
                                        'md-datetime--active': cell.active,
                                        'md-datetime--selected': cell.selected,
                                    })}"
                                >${cell.label}</td>
                            `)}
                        </tr>
...
// test
class MdWeekPicker extends MdDatetimePickerElement {
    constructor() {
        super();
        this.type = "week";
    }
            ...
            <table>
                ...
                <tbody>
                    ${this.calendar.map(row=>html`
                        <tr
                            class="${classMap({
                                'md-datetime--active': row.active,
                                'md-datetime--selected': row.selected,
                            })}"
                        >
                            <td
                                class="${classMap({
                                    'md-datetime--outside-month': row.outsideMonth,
                                })}"
                            >${row.week}</td>
                            ${row.children.map(cell=>html`
                                <td
...
src\material\base\layout-item.js
...
class MdLayoutItem extends MdElement {
    ...
    connectedCallback() {
        ...
        this.scrimElement.on("scrimClose", this._handleLayoutItemScrimClose);
    }

    disconnectedCallback() {
        super.disconnectedCallback();

        this.scrimElement.off("scrimClose", this._handleLayoutItemScrimClose);
    ...
    _handleLayoutItemTransitionend(event) {
        if (this.open) {
            this.emit("layoutItemShowed", { element: this });
        } else {
            this.emit("layoutItemClosed", { element: this });
        }
    }
    ...
    show() {
        this.open = true;
        this.emit("layoutItemShow", { element: this });
    }

    close() {
        this.open = false;
        this.emit("layoutItemClose", { element: this });
    }
...
class MdListElement extends MdElement {
    static properties = {
        ...
        type: { type: String }, // list | tree | stack
    ...
    //     if (_changedProperties.has("items")) {
    //         console.log(this.items,this._list,this._tree,this._stack);
    //     }
    ...
    //     if (_changedProperties.has("_tree")) {
    //         console.log(this._tree);
    //     }

    //     if (_changedProperties.has("_stack")) {
...
class MdScrim extends MdElement {
    ...
    connectedCallback() {
        ...
        this.on("click", this.handleScrimClick);
    }
    ...
    disconnectedCallback() {
        ...
        this.off("click", this.handleScrimClick);
    ...
    _handleScrimTransitionend(event) {
        if (this.open) {
            this.emit("scrimShowed", { element: this });
        } else {
            this.emit("scrimClosed", { element: this });
        }
    }
...
:root {
    --md-sys-color-on-surface10: color-mix(in srgb, var(--md-sys-color-on-surface) 10%, transparent);
...
src\material\base\typography.scss
...
class MdCheckbox extends MdElement {
    ...
    _updateValidationClass() {
        this.classList.toggle("md-checkbox--error", this.error);
    }

    validate() {
        const checkboxNative = this.checkboxNative.value;
        this.errorText = checkboxNative.validationMessage;
...
//     -webkit-box-shadow: 0 0 0 1000px var(--md-sys-color-surface-container-highest) inset !important;
...
export { MediaObserver, BreakpointObserver, SchemeObserver, OrientationObserver };
```
...
// base components
import "./base/scrim.js";
import "./base/layout.js";
import "./base/layout-item.js";
import "./base/icon.js";
