...
var MdTreeComponent = /** @class */ (function (_super) {
    ...
    MdTreeComponent.prototype.render = function () {
        var _this = this;
        return this.storeItems.filter(function (item) { return item.visible; }).map(function (item) { return _this.renderTreeItem(item); });
    };
    /**
     */
    ...
    MdTreeComponent.prototype.updateScroll = function (arg) {
        return __awaiter(this, void 0, void 0, function () {
            var treeItemSelected;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.updateComplete];
                    case 1:
                        _a.sent();
                        treeItemSelected = this.querySelector("md-tree-item[selected]");
    ...
    MdTreeComponent.prototype.handleTreeKeydownArrowDown = function (event) {
        ...
        var visibleItems = this.storeItems.filter(function (item) { return item.visible; });
        var selectedIndex = visibleItems.findIndex(function (item) { return item.selected; });
        if (selectedIndex === visibleItems.filter(function (item) { return item.visible; }).length - 1)
            return;
        var nextItem = visibleItems.find(function (item, index) { return item.visible && index > selectedIndex; });
    ...
    MdTreeComponent.prototype.handleTreeKeydownArrowUp = function (event) {
        event.preventDefault();
        var visibleItems = this.storeItems.filter(function (item) { return item.visible; });
        var selectedIndex = visibleItems.findLastIndex(function (item) { return item.selected; });
        if (selectedIndex === 0)
            return;
        var nextIndex = visibleItems.findLastIndex(function (item, index) { return item.visible && index < selectedIndex; });
        visibleItems.forEach(function (item, index) {
    ...
    MdTreeComponent.prototype.handleTreeKeydownArrowRight = function (event) {
        var _a;
        event.preventDefault();
        var selectedItem = this.storeItems.find(function (item) { return item.selected; });
        if ((_a = selectedItem.children) === null || _a === void 0 ? void 0 : _a.length) {
            selectedItem.expanded = true;
            this.updateVisibility(selectedItem);
            var visibleItems = this.storeItems.filter(function (item) { return item.visible; });
            var selectedIndex_1 = visibleItems.findIndex(function (item) { return item.selected; });
            if (selectedIndex_1 === visibleItems.filter(function (item) { return item.visible; }).length - 1)
                return;
            var nextIndex_1 = visibleItems.findIndex(function (item, index) { return item.visible && index > selectedIndex_1; });
            visibleItems.forEach(function (item, index) {
    ...
    MdTreeComponent.prototype.handleTreeKeydown = function (event) {
        if (this.contains(document.activeElement)) {
            if (event.key === "ArrowLeft")
                return this.handleTreeKeydownArrowLeft(event);
            else if (event.key === "ArrowUp")
                return this.handleTreeKeydownArrowUp(event);
            else if (event.key === "ArrowRight")
                return this.handleTreeKeydownArrowRight(event);
            else if (event.key === "ArrowDown")
                return this.handleTreeKeydownArrowDown(event);
            else if (event.key === "Enter")
                return this.handleTreeKeydownEnter(event);
        }
